...
We will set a term at the top that will denote whether the user wants to order and produce a gif from which folder.

```{r}
# Options: tsne_plots, umap_plots, merged_plots
out_folder <- "merged_plots"
```
...
library(here)
library(magick)
setwd(here::here('output', out_folder))
...
```{r}
files <- list.files(here::here("output", out_folder))
new_order <- files[ph$tree_row$order]
new_order <- paste0(here::here("output", out_folder), "/", new_order)
```

Now we make the gif based on that new order. We note that in the contexting of producing the tsne-umap merged plots, the order of the umap does not matter so much, so we work directly in the merged images and just get the t-sne in the right order.
...
out_file <- paste0("ordered_", out_folder, ".gif")
MakeGif(in_dir = here::here('output', out_folder), outfile = out_file, fps = 5) 
```
